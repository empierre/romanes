			...
			foreach my $v (@l) {
				...
				while ($sth->fetch()) {
					...
					if (!$mem_dep{$plid}) {
						...
						my %ix=();
						%ix=('thb_url'=>"http://www.romanes.org/thumb/$tf",'place_name_fr'=>$nm,'album_url'=>$hosting.$album_url,'town_name_fr_1'=>$town_name,'epoch'=>$epoch_str,'style'=>$epoch_style,'BGC'=>'#E6E6D2');
